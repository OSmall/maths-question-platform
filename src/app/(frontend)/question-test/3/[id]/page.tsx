...
import { Attempt3Renderer } from '@/components/question-test/attempt-3-renderer'
...
export default async function QuestionTest3Page({ params }: QuestionTestPageProps) {
  ...
  return questionResult.match(
    (question) => (
      <div className="min-h-screen w-full bg-[radial-gradient(circle_at_20%_10%,_#e1f9f0_0%,_#f4fffb_38%,_#ffffff_100%)] px-4 py-8 md:px-6 md:py-10">
        {isDraftMode ? <RefreshRouteOnSave /> : null}
        <div className="mx-auto flex w-full justify-center">
          <Attempt3Renderer question={question} />
        </div>
      </div>
    ),
    (err) => {
