...
import { Attempt11Renderer } from '@/components/question-test/attempt-11-renderer'
...
export default async function QuestionTest11Page({ params }: QuestionTestPageProps) {
  ...
  return questionResult.match(
    (question) => (
      <div className="from-background via-muted/20 to-background min-h-screen w-full bg-gradient-to-b px-4 py-8 md:px-6 md:py-10">
        {isDraftMode ? <RefreshRouteOnSave /> : null}
        <div className="mx-auto flex w-full justify-center">
          <Attempt11Renderer question={question} />
        </div>
      </div>
    ),
    (err) => {
