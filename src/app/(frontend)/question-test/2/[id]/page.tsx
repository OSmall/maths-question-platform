...
import { Attempt2Renderer } from '@/components/question-test/attempt-2-renderer'
...
export default async function QuestionTest2Page({ params }: QuestionTestPageProps) {
  ...
  return questionResult.match(
    (question) => (
      <div className="min-h-screen w-full bg-[radial-gradient(circle_at_top,_#f6efe2_0%,_#f3ebdc_45%,_#ece2cf_100%)] px-4 py-8 md:px-6 md:py-10">
        {isDraftMode ? <RefreshRouteOnSave /> : null}
        <div className="mx-auto flex w-full justify-center">
          <Attempt2Renderer question={question} />
        </div>
      </div>
    ),
    (err) => {
